...
- Если существует `openspec-orch.yaml`, используйте его `repositories` как реестр
  точных `repository-id`; связи репозиториев и Specs читайте из валидированного
  `openspec/graph.yaml`, а локальное техническое устройство — только в самих Code
...
| Архитектура, компоненты и интеграции | `03-architecture.md`, `openspec/graph.yaml`, `ADR/` |
| Репозитории, Specs и их связи | `openspec-orch.yaml`, `openspec/graph.yaml` |
