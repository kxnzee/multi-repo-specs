...
её нативную регистрацию. `--refresh` показывает ход операции, явно обновляет уже
установленную копию и сверяет результат. Если штатный native update сохранил устаревшие
файлы той же версии, команда один раз переустанавливает Extension из зафиксированного
источника и повторяет сверку. Успешный `connect` выводит только итог операции и не
дублирует полный нативный список Extensions для каждого целевого Repository.
