...
Для включения профиля в начальный набор новых Store добавьте его в `templates/base/subagents/`. Базовый Template установит файл в каталог `agents/` выбранного adapter. Расширять `openspec/config.yaml` и список обязательных профилей при этом не нужно.
