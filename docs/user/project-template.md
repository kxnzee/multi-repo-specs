...
Обе schemas используют одну Feature Acceptance: Agent готовит короткую выжимку и
воспроизводимые проверочные кейсы, человек проходит их и явно принимает решение
`PASS` или `FAIL`, а до решения gate остаётся `PENDING`.
...
указывается одной ссылкой на канонический реестр, а SHA, история веток, timestamps,
сырые логи, ответы API и рассуждения агента в него не копируются.
...
ссылка на постоянное evidence; stdout команды и диагностика окружения туда не
