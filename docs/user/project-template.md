...
учитывается по ID или точной ссылке на Specs, без копирования его описания.
...
неприменимость — `N/A` с причиной. Можно ссылаться на общий отчёт, если он покрывает
каждый сценарий и однозначно указывает проверенные версии. Для межрепозиторных
сценариев проверяется взаимодействие этих версий, а не только отдельные репозитории.
Пропуски, `FAIL` и `PENDING` не позволяют объявлять результат принятым.
