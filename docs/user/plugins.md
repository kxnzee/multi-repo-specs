...
Обычный `status` показывает задачу, Repository, состояние записи, актуальную
галочку и соответствие checkout — без SHA и журналов. `complete` в карте не равно
успешному Verify. `ahead` означает, что checkout уже содержит более поздний commit,
а не точное совпадение с записанной реализацией.

Для машинных данных есть `status <change-id> --json` и `tracking_status`.
Ответ включает текущий `candidate` только для репозиториев, известных Tracking;
полноту scope нужно сверить с Change. Перед проверками сохраните этот снимок
отдельным локальным evidence-файлом в Store, например
`openspec/changes/<change-id>/evidence/verify-01-candidate.json`.
Не перезаписывайте его после нового checkpoint. В `verify.md` достаточно локальной
ссылки на снимок, сценариев, воспроизводимых шагов и результатов. Ни PR-ссылки,
ни ручной ввод SHA не нужны. Tracking сам не запускает проверки и не сохраняет
результат Verify.
