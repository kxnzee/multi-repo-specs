...
Выбирайте схему по нужной дисциплине исполнения. Обе схемы поддерживают изменения
в нескольких репозиториях: число репозиториев и сложность задачи сами по себе
не определяют выбор. Схема выбирается отдельно для каждого Change.

Если команда не требует обязательного подробного плана, TDD и промежуточных ревью
агента, начинайте с **расширенного OpenSpec (`spec-driven-extended`)**. Это
рекомендация для выбора, а не автоматическое переключение настроек проекта.
Схема помогает уточнить требования через Intent и Intake, согласовать решение,
разбить работу на проверяемые задачи и принять результат. Способ организации
реализации определяется процессом команды.

Выбирайте **Superspec на базе Superpowers (`superspec-multirepo`)**, когда хотите
заранее задать агенту подробный порядок разработки: одобренный Brainstorm,
отдельный Plan, изолированную рабочую копию, TDD, промежуточные ревью и проверку
соблюдения процесса. Учитывайте дополнительную работу по согласованию крупных
задач в `tasks.md` и подробных шагов в `plan.md`.

| Ваш случай | Рекомендуемая схема | Основание выбора |
|---|---|---|
| Есть бизнес-запрос, нужно уточнить сценарии, ограничения и критерии приёмки | `spec-driven-extended` | Intent и Intake сохраняют подтверждённые исходные требования перед Planning |
| Аналитик готовит Planning, разработчики организуют реализацию по процессу команды, тестировщик проверяет результат | `spec-driven-extended` | Достаточно общего контракта, Design, проверяемых Tasks и Verify |
| Дорабатывается существующая функция; отдельный подробный план исполнения агентом не нужен | `spec-driven-extended` | Задачи уже содержат результат и способ проверки; сложность изменения сама по себе не требует Superspec |
| Агенту поручается реализация с обязательным TDD и промежуточными ревью | `superspec-multirepo` | Эти практики входят в правила Apply выбранной схемы |
| Нужны выполнение подробного плана через субагентов и ревью задач | `superspec-multirepo` | Предусмотрены субагенты, а при их недоступности — пакетное выполнение с сохранением TDD и ревью |
| Нужно отдельно подтвердить соблюдение процесса разработки | `superspec-multirepo` | Verify включает Process Compliance наряду с человеческой Feature Acceptance |

Например, новый способ оплаты во frontend и backend можно реализовать по обеим
схемам. Выберите расширенный OpenSpec, если команда согласует поведение и задачи,
а разработчики организуют выполнение. Выберите Superspec, если для того же изменения
нужен подробный план работы агента с обязательным TDD и промежуточными ревью.
Параллельная работа в Superspec допустима только для независимых задач.

В обеих схемах агент может выполнять реализацию, а требования, важные решения и
приёмка остаются под контролем человека. Инструкции Superpowers задают порядок
работы; его соблюдение нужно подтверждать фактическими проверками.
...
| В `spec-driven-extended` техническое решение простое | Подготовить обязательный перед Tasks краткий Design: указать отсутствие новых архитектурных решений и заполнить Repository Implementation Map |
| В `superspec-multirepo` техническое решение простое и нет условий, требующих Design | Пропустить опциональный Design; его доступность сама по себе не требует создания файла |
