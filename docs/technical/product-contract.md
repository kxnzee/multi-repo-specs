...
обновления относящейся документации и проверки публичного поведения. Pilot feedback и
`BACKLOG.md` используются как evidence и очередь кандидатов, но не являются
обязательным предварительным условием каждого изменения.
...
отдельного решения владельца и обновления контракта; pilot feedback является одним из
допустимых evidence.
...
нет — pilot evidence покажет, бывают ли замены и гонки вообще. Строгая цепочка может
быть добавлена только отдельным принятым Change.
...
Ревизия пилота уточняет приоритеты, но не замораживает Core и не является единственным
механизмом изменения контракта.
