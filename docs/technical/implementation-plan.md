...
Статус: **v1 реализована и заморожена до ревизии пилота**.

Нормативный источник: [продуктовый контракт](product-contract.md).
...
контракт в текущую версию не входят.
...
Причина ошибки печатается в stderr одним из стабильных кодов раздела 6 продуктового
контракта.

## 2. Ограничения продукта и выбор реализации

Продуктовый контракт задаёт ограничения продукта. Этот план отдельно фиксирует сменяемые
технические решения текущей Node.js-реализации:

| Ограничение продукта | Выбор реализации v1 |
|---|---|
...
1. Зафиксировать документы текущей версии и реализацию в Git. Во время пилота эти документы
...
4. Зафиксировать расхождения текущего прототипа и продуктового контракта:
...
Цель: передать v1 одному инженеру без дополнительного устного проектирования.
...
2. README, который чётко разделяет текущий реализованный CLI и архив полной
   модели.
3. Актуальный пример `openspec-orch.yaml` и команды установки.
4. `docs/user/pilot-feedback.md` с тремя вопросами из продуктового контракта.
...
v1 готова к пилоту, только если выполнены критерии выхода этапов 0–4 и четыре
неснижаемые проверки раздела 2.3 продуктового контракта. Публикация пакета, второй командный
пилот, migration plan и Beta Plan не являются условиями готовности v1.
