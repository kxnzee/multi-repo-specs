...
С момента фиксации этой редакции и до ревизии пилота (раздел 7) контракт альфы заморожен: новые идеи не кодируются, а записываются в `docs/user/pilot-feedback.md`.
...
Сценарии показывают, как альфа используется в бою. Оба обязаны выполняться только реализованными операциями; если в ходе пилота шаг сценария не удаётся выполнить без ручной работы — это запись в `docs/user/pilot-feedback.md`.
