...
наблюдаемый ожидаемый результат, агрегированный статус агента и отдельное решение
...
описывается человеческим языком, без Repository ID, checkout, путей, имён файлов,
symbols, команд, хешей и URL. Для автоматических проверок записываются только тип,
количество выполненных проверок и общий статус. Имена отдельных тестов, ссылки на
тесты или отчёты, сырые логи, ответы API и рассуждения агента в Verify не попадают.
Если кандидат или количество выполненных проверок неизвестны, агент ставит
`BLOCKED`, а не восстанавливает данные через API хостинга.
...
агрегированное количество выполненных проверок по типам; технические ссылки,
команды, пути, стандартный вывод и диагностика окружения туда не переносятся.
Решение человека и будущие Archive/UAT/Release остаются отдельно от подтверждений.

[Создание минимального Template через CLI](../user/creating-addons.md).
