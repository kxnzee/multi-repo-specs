...
Статус: **готов к freeze перед реализацией Alpha v1**.

Нормативный источник: [Alpha Concept](OpenSpec-Orchestrator-Alpha-Concept.md).
...
[runbook](OpenSpec-Orchestrator-Alpha-Pilot-Runbook.md). Новые функции до ревизии
`docs/alpha-feedback.md` не добавляются.
...
1. Зафиксировать три Alpha-документа отдельным документационным коммитом и записать
   его SHA как `Alpha Pilot Contract SHA`. Во время реализации эти документы не
   редактируются; наблюдения идут только в `docs/alpha-feedback.md`.
2. Зафиксировать SHA реализации и полный результат `npm run check`.
...
1. До начала пилота выбрать реальный Change: название и задача, владелец, Store,
   frontend и backend repository IDs, а также понятный критерий завершения.
...
4. `docs/alpha-feedback.md` с тремя вопросами из Alpha Concept.
5. Пилот по [runbook](OpenSpec-Orchestrator-Alpha-Pilot-Runbook.md) на Store,
   frontend и backend.
