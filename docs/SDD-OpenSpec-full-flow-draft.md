...
После успешной подготовки CLI печатает готовую строку с оригинальным действием OpenSpec и SDD-инструкцией. Пользователь целиком копирует её в Qwen Code:

```text
/opsx-explore <ticket-id>. Это Explore шага 01 SDD. Используй Spec Root ...
```

`sdd explore` проверяет формат ticket key и отсутствие точного дубликата, получает через OpenSpec список Master Specs, предлагает интерактивно выбрать Code Repositories и при необходимости атомарно публикует их свежие checkout по детерминированному пути ticket. Пустой выбор означает Explore только по `project-specs`; неинтерактивный запуск запрещён. OpenSpec сам не управляет свежестью Git, поэтому свежесть выбранного кода обеспечивает `sdd`. Содержимое `.qwen/commands/opsx-explore.md` принадлежит OpenSpec: `sdd explore` его не изменяет и не проверяет по fingerprint. Отсутствующий action восстанавливается штатным `openspec update --force` до повторного запуска.

#### Назначение команд

| Команда | Что делает | Результат |
|---|---|---|
| `sdd explore --ticket <ticket-id>` | Проверяет среду, key и существующие Changes, интерактивно выбирает Code Repositories, при необходимости получает их свежие ревизии и формирует полный ввод для агента | Атомарно подготовленный read-only workspace без manifest и готовая строка `/opsx-explore ...` |
| Готовая строка `/opsx-explore ...` | Запускает оригинальный action с переданными SDD-границами, читает запрос, нормативные источники и подготовленный код; не создаёт Change и не изменяет файлы | Структурированный итог для решения Change Owner |
...
5. Целиком скопировать напечатанную строку `/opsx-explore ...` в Qwen Code, прочитать назначенный нормативный контекст, подходящие Master Specs и `Purpose` capabilities.
...
В будущем этот handoff может быть отдельно согласованным изменением переведён на собственную `/sdd-explore`, если передачи SDD-контракта аргументом станет недостаточно. Такая команда должна использовать пространство имён `sdd-*`, не перезаписывать `opsx-*` и по возможности сохранить контракт подготовки workspace. До такой ревизии используется только оригинальный `/opsx-explore` с полной инструкцией от CLI.
...
Пользователь не собирает flow из низкоуровневых команд OpenSpec вручную: адаптер `sdd` подготавливает их ввод и проверяет структурированный результат. На шаге 01 технически необходим явный handoff между CLI и текущей Qwen-сессией, поэтому пользователь копирует одну полностью сформированную строку `/opsx-explore ...`; её части вручную не конструируются. Beta Stores, references, working context и worksets намеренно не используются из-за нестабильного beta-контракта. Нормативный multi-repo контракт принадлежит `sdd load`, `sdd open`, `sdd verify load` и `sdd verify open`; внутри они опираются только на Git, точные ревизии и проверенные manifest. OpenSpec root используется для планирования и read-only Baseline, но не определяет область записи реализации. После стабилизации beta-функций их можно подключить как сменный адаптер, не меняя шаги процесса и формат карточки.
