...
flowchart TD
    ...
    N01 -->|"общий контекст устарел"| N01
...
| `/sdd-context` | Сначала инвентаризирует Markdown-файлы `openspec/context/`, затем обрабатывает `_raw/**/*.md` и только после этого остальные пробелы | Предлагает diff context files, ADR и examples для ревью владельцев |
...
2. классифицирует пути как нормативный context, ADR, example, служебный README или raw-материал и отмечает отсутствующие элементы skeleton;
...
10. после полного прохода обновляет в `_raw/README.md` только строку `Last scanned: YYYY-MM-DD`;
11. после обработки или явного откладывания raw-файлов проходит inline TODO и задаёт адресные вопросы по оставшимся разделам skeleton;
12. маршрутизирует устойчивые факты в context files, решения — в ADR, иллюстрации — в examples, а неизвестное — в TODO целевого файла;
13. обновляет `00-start-here.md` и индексы ADR/examples, показывает оставшийся diff по каждому файлу и применяет только подтверждённые изменения.
...
Дата `Last scanned` означает только момент последней полной инвентаризации `_raw/`, а не принятие всего его содержимого. При каждом вызове raw-файлы перечитываются и сравниваются с нормативным контекстом: уже перенесённый факт не дублируется, а новое, изменённое или противоречащее утверждение снова уточняется у пользователя.
...
Файлы `00-start-here.md`–`08-release-process.md`, `system-map.yaml` и три README создаются с готовой структурой, инструкциями и локальными `TODO`, а не пустыми. `sdd init` не проверяет и не перезаписывает существующее содержимое: он добавляет только отсутствующие элементы skeleton bundle.
...
| `01-product-context.md` | Назначение продукта, пользователи, границы и бизнес-ограничения | Change Owner |
| `02-domain-glossary.md` | Только термины с единым нормативным значением | Change Owner |
...
| `ADR/README.md` и ADR | Правила оформления, индекс и принятые долговечные архитектурные решения | Technical Owner |
| `examples/README.md` | Правила, индекс и раздел `Planned examples` для ненормативных примеров | Spec Owner |
| `_raw/README.md` и raw-файлы | Правила, дата последнего сканирования и неподтверждённые исходные материалы | Автор материала; дату сканирования обновляет `/sdd-context` |
...
Если обнаружен пробел, `/sdd-context` записывает inline TODO с владельцем и ожидаемым источником в целевой context-файл. Пока целевой файл неизвестен, вопрос остаётся в `Open questions` файла `00-start-here.md`. Подтверждённый устойчивый факт переносится в соответствующий нормативный файл, долговечное решение — в ADR, а полезная иллюстрация — в `examples/`. Неподтверждённое остаётся в `_raw/`. Требования конкретной фичи и Delta Specs команда не создаёт.

Агент не создаёт examples заранее и не выводит их только из собственного знания. Пример появляется после подтверждённого правила, сложного сценария или повторяющегося вопроса; он ссылается на нормативный раздел, который иллюстрирует, и явно помечается как ненормативный.
...
Шаг выполняется для каждой новой Jira Story после завершения шага 00. Он проверяет запрос и актуальность общего контекста, затем исследует проблему без создания `openspec/changes/` и записи в `state.yaml`.

### Владельцы

Change Owner отвечает за Jira Story, проблему и ожидаемый результат; Technical Owner — за техническое исследование; Spec Owner следит, чтобы решения и неизвестные не остались только в чате.
...
Если общий контекст отсутствует, предположительно устарел или его актуальность нельзя подтвердить, в агентском чате сначала вызывается:

```text
/sdd-context
```

После успешной предпроверки используется действие OpenSpec, подключённое адаптером:

```text
/opsx-explore
```

`sdd explore` проверяет тикет, получает через OpenSpec список Master Specs, очищает временный каталог, получает свежие состояния выбранных репозиториев и показывает агенту пути только для чтения. OpenSpec сам не управляет свежестью Git, поэтому её обеспечивает `sdd`. Beta-команда `openspec context` не используется: её working context не связан с `openspec/context/` проекта.

#### Назначение команд

| Команда | Что делает | Результат |
|---|---|---|
| `/sdd-context` | Сверяет context pack и Master Specs со свежим состоянием зарегистрированных репозиториев, показывает diff и применяет только подтверждённые изменения | Статус `current` либо согласованный `updated` |
| `sdd explore --ticket <ticket-id>` | Проверяет тикет, получает через адаптер OpenSpec сведения о Master Specs и открывает свежие code repo только для чтения | Временный исследовательский контекст без Change |
| `/opsx-explore` | Использует установленное OpenSpec action для исследования проблемы, альтернатив и неизвестных | Подтверждённые выводы и вопросы в сессии Explore |
...
1. Убедиться, что шаг 00 завершён и работа начинается по существующей Jira Story.
2. Зафиксировать key Story как `<ticket-id>` и проверить, нет ли активного Change с той же целью.
3. Проверить наличие и читаемость `openspec/context/00-start-here.md` и назначенных им файлов.
4. Проверить, не менялись ли границы продукта, доменная модель, архитектура, безопасность, межсистемные инварианты, quality gates или процесс поставки.
5. При пробеле или сомнении выполнить `/sdd-context`; после обновления заново прочитать затронутый контекст.
6. Прочитать подходящие Master Specs и `Purpose` capabilities.
7. По `system-map.yaml`, `sdd.yaml`, Master Specs и свежему коду определить репозитории-кандидаты и внешние системы.
8. Исследовать код, контракты, существующие точки расширения, альтернативы и неизвестные.
9. Если во время Explore обнаружился новый пробел общего контекста, повторить действия 3–5 внутри этого же шага.
10. Change Owner принимает одно решение: продолжить, вернуть запрос на уточнение или закрыть его.

### Результат и гейт 01

Результат шага пока не является Change. Его постоянная запись находится в Jira Story; сессия Explore служит рабочей средой исследования.

Продолжение разрешено, когда:

- Jira Story существует, её key зафиксирован;
- проблема и ожидаемый результат понятны;
- обязательный контекст существует, прочитан и имеет статус `current` или согласованный `updated`;
