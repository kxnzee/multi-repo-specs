# OpenSpec Orchestrator: концепт альфа-версии (alpha v1)

## 0. Статус и назначение документа

Статус: **зафиксированный контракт альфы; редакция 3 — перед запуском пилота**.

> **Назначение альфы.** Alpha v1 предназначена не для доказательства полной архитектуры Orchestrator, а для проверки одной гипотезы в реальной работе одного инженера: сохранение Cycle-контекста (какие репозитории и какая версия планирования приняты для изменения) и воспроизводимый Snapshot (какой точный набор коммитов проверялся) дают ощутимую ценность. В альфе намеренно нет агентских handoff, автоматических проверок, multi-user и доказательной цепочки — их отсутствие является решением, а не недоработкой.

Этот документ — единственный актуальный продуктовый контракт Orchestrator Alpha.
...
они не дополняют Alpha Concept и не имеют приоритета при конфликте.
...
С момента фиксации этой редакции и до ревизии пилота (раздел 7) контракт альфы заморожен: новые идеи не кодируются, а записываются в `docs/alpha-feedback.md`.
...
Альфа отвечает ровно на два вопроса:
...
Всё остальное находится вне альфы. Вернуться к этим функциям можно только после
пилота и при наличии записанной пользовательской боли.

### 1.1. Ограничение: один пользователь, одна рабочая копия Store

Alpha v1 предполагает **одного пользователя и одну активную рабочую копию Store на машине**. Параллельная работа нескольких пользователей не поддерживается.

Практическое следствие: текущий Cycle определяется по HEAD рабочей копии Store (раздел 3.3), поэтому два человека на разных ветках одного Store могут видеть разные Cycle. Это не баг, а зафиксированное ограничение альфы. Cycle Record переносится между машинами через обычный Git; Result Receipts, Snapshots и Verification Receipts остаются локальными на каждой машине и через Git не переносятся. Как в этих рамках распределить роли между людьми — описано в сценарии 5.2.

### 1.2. Критерий успеха альфы

Альфа удалась, если после 3–5 реальных изменений через 2–3 репозитория выполняется:
...
5. Заполнен `docs/alpha-feedback.md`, и по его записям на ревизии принимается решение о составе беты.
...
Требование к ним в рамках альфы: привести к конфигурации из раздела 3.2 (строгая версия, реестр репозиториев), если ещё не соответствуют. `connect` не делает pull/rebase/checkout и не переписывает конфигурацию.
...
| Операция | Что делает | Чего не делает в альфе |
|---|---|---|
...
Альфа без этих проверок — обёртка вокруг ручного процесса, и пилот измерит ноль.

### 2.4. Не входит в альфу
...
Отсутствие этих функций — не дефект альфы. До ревизии пилота список не кодируется.
После ревизии функция может попасть в отдельный Beta Plan только против записи в
`docs/alpha-feedback.md` (раздел 7).

## 3. Модель данных альфы

Четыре класса данных — зафиксированная структурная граница альфы:
...
Receipts и Snapshots **никогда** не попадают в Git в альфе. Cycle Record **никогда** не хранится только в state.json.

### 3.1. Два обязательных задела на будущее

В альфе есть ровно два резерва совместимости. Оба дёшевы сейчас и предотвращают болезненную миграцию данных после пилота:
...
- в строгом режиме неизвестные поля вне `extensions` — ошибка; это относится и к не входящим в альфу секциям `agent` и `handoffs`;
...
- `cycle_id` — настоящий уникальный ID, генерируется при каждом `assign`, даже с примитивной активацией альфы;
...
Дополнительно, если репозиторий определяется текущим каталогом: предупреждение при несовпадении HEAD рабочей копии с заявленным коммитом (не ошибка — в альфе пользователь может записывать результат из другого каталога).

Незакоммиченные изменения, stash и diff версией реализации не являются.

**Замена результата.** Новый Receipt для той же пары `cycle_id + repository_id` заменяет предыдущий; предыдущий сохраняется в истории state.json; пользователь получает предупреждение «результат заменён». Поля `supersedes` в альфе нет — пилот покажет, бывают ли замены и гонки вообще. Строгая цепочка может быть добавлена только отдельным решением после пилота.
...
Это правило — **не упрощение, а ядро ценности**, и оно фиксируется в альфе навсегда: время, пути, имя машины в идентичность не входят. Повторный `verify` с теми же входами возвращает тот же `snapshot_id`.

**Упрощение альфы — без развёртывания.** `verify` не создаёт worktree. Он печатает таблицу «репозиторий → точный коммит» и инструкцию: проверяющий сам делает checkout. Ответственность Core — только детерминированная идентичность набора версий.
...
- Receipt передаётся флагами CLI (`--commit`, `--status`, `--source`, `--note`); файл/stdin в альфу не входят.
...
Сценарии показывают, как альфа используется в бою. Оба обязаны выполняться только реализованными операциями; если в ходе пилота шаг сценария не удаётся выполнить без ручной работы — это запись в `docs/alpha-feedback.md`.
...
Важно для пилота: количество ручных шагов в этом сценарии — главный источник записей в `docs/alpha-feedback.md` и главный материал для решения о составе беты.
...
Обратная связь ведётся в одном обычном файле `docs/alpha-feedback.md`, вручную, без отдельного формата и процесса. Записывается каждый случай по трём вопросам:
...
До ревизии контракт альфы не меняется и backlog не кодируется.
...
- **MVP-1 — учёт результатов:** `state.json`, Result Receipt, `record assignment` с проверками. После этого шага связка «Cycle + Receipts + status» отвечает на первый вопрос альфы и демонстрируема — это точка отсечения при нехватке времени.
- **MVP-2 — проверка:** `verify`, Snapshot, `record verification`. Отвечает на второй вопрос альфы.
- **Далее — пилот.** Ничего не добавлять.

## 9. Сводная таблица упрощений

| Узел | Альфа | Возможное развитие после пилота | Что заложено для перехода |
|---|---|---|---|
| Активация Cycle | HEAD рабочей копии Store | нормативный путь в дереве основной ветки, `pending/committed_pending/active` | полный формат Cycle Record уже в альфе |
