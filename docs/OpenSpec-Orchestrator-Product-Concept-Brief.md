# OpenSpec Orchestrator: актуальная продуктовая концепция и scope lock v1

## 0. Статус и назначение документа

Статус документа: **актуальный продуктовый и архитектурный scope lock v1**.
...
- с документом из `docs/archive/` — актуален этот документ;
...
(init | adopt) → first local connect
    ↓
Change + accepted Change Binding + Baseline
    ↓
status(change-id)
    ↓
...
7. Принятый Change Binding со списком участвующих repository IDs.
8. Автоматическое разрешение и показ Planning Baseline.
...
Роль Template зависит от явно вызванной операции Repository Layer.
...
- указывает существующие implementation и verification handoffs;
...
1. implementation handoff получает сформированный Core Implementation Context;
2. verification handoff может проверить согласованный Snapshot;
3. результат реализации нормализуется в versioned Result Receipt;
4. OpenSpec bridge использует публичные возможности OpenSpec, не читая внутренний
...
Handoffs проверяются лениво. Отсутствие implementation handoff не мешает выполнить
...
```text
Repository Layer
init | adopt ──→ first local connect ──→ repository status/sync ──→ disconnect
                       ↑
connect ───────────────┘
                       │
                       ▼
Change Layer
plan through OpenSpec/Template → accept Binding + Baseline → derive Assignments
                         │
                         ▼
Assignment/Implementation Layer
prepare context → implementation handoff → record Receipt → verify exact Snapshot
```
...
реализацию для каждого repository из принятого Binding.
...
6. Не требует implementation/verification handoff для завершения Repository Layer:
...
| Info | Необязательный implementation/verification handoff пока не настроен |
...
- меняются implementation/verification handoffs;
...
Имена и назначение `init`, `adopt` и `connect` зафиксированы scope lock. Implementation
plan может уточнить синтаксис остальных операций и флагов, но их identities и
семантика не должны меняться от команды к команде без причины.
...
1. Change создаётся и развивается штатными механизмами OpenSpec через процесс
   Template.
2. Пользователь задаёт известный список repositories.
3. Planning-agent может предложить изменение списка с объяснением.
4. Пользователь подтверждает Change Binding.
5. Planning artifacts и Binding принимаются на одном Baseline.
6. Core может вывести Assignments и aggregate status.
...
10. Результат фиксируется через Result Receipt на точной revision.
11. Цикл повторяется для остальных repositories.

### 9.4. Проверка Change

1. `status(change-id)` показывает все Assignments.
2. Core проверяет наличие актуальных Receipts.
3. Собирает Snapshot точных implementation revisions.
...
7. Существующие implementation/verification commands связываются с handoffs.
...
- Result Receipt;
- Snapshot;
- JSON contracts status/implement/record/verify;
- warning/error/confirmation protocol;
- compatibility с текущими handoffs;
- public contracts `init`, `adopt` и `connect`;
...
1. Реализовать Change Binding.
2. Разрешать accepted Baseline автоматически.
3. Создавать Assignment identities.
4. Реализовать aggregate `status(change-id)`.
5. Добавить stale detection при изменении Binding/Baseline/config.
6. Сохранить OpenSpec Schema opaque.
...
8. Реализовать Snapshot и `verify(change-id)`.
...
11. Существуют три слоя: Repository, Change и Assignment/Implementation.
