...
При `init` версия config и роли Store/Code назначаются автоматически. Поля `plugins`
и `repositories[].plugins` меняются только командами Plugin lifecycle, а подключённые
Store с ролью `specs` добавляются отдельным изменением переносимого config. Поэтому
эти поля не дублируются параметрами `init`.
...
Команда может заполнить `repositories[].description` во время `init`: интерактивно,
через CLI-флаги `--store-description`/`--repo-description` или поля MCP
`store_description`/`repositories[].description`. Позже описание можно изменить
вручную в `openspec-orch.yaml`. Ориентир — 2–4 предложения:
...
кода и передать согласованный текст в `initialize_project`. Команда поддерживает
актуальность описаний и проверяет изменения обычным review в Store. `init` только
сохраняет переданный текст, а `connect` и `doctor` не генерируют и не обновляют
описания автоматически. Отдельной команды генерации нет.
...
## Вход в реализацию по Tasks

Схема `superspec-multirepo` передаёт `writing-plans` шаблон Tasks и сохраняет его
результат только в `tasks.md`. Этот файл одновременно задаёт стабильные OpenSpec
task ID, порядок репозиториев, небольшие шаги, проверки и review checkpoints.
Отдельный `plan.md` не создаётся.

Штатный Apply передаёт `tasks.md` в `subagent-driven-development` или
`executing-plans` как принятый план и сохраняет TDD, review, systematic debugging
и verification-before-completion. Существующие локальные схемы Store автоматически
не переписываются; при обновлении перенесите новый граф и шаблон Tasks вручную.
...
Git Flow, ветки и направления PR принадлежат процессу команды. Orchestrator не
создаёт его описание и использует только явно предоставленные правила проекта.
