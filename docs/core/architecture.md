...
- Change Tracking хранит текущую связь задачи и Code Repository с автоматически
  полученными Git revisions в Change-local `implementation-map.yaml`.
  Локальная запись начала работы защищает историю и конкурентное обновление;
...
использует общий MCP для Store context. Плагин читает task progress через публичный
JSON API OpenSpec, но не меняет и не блокирует шаги; workflow одинаково работает
с ним и без него.
...
- controlled setup tools `initialize_project` и `connect_project` только для
  fixed-cwd flow;
- optional Tracking tools из Plugin: start, checkpoint, complete, cancel и status;
...
непрозрачный ключ, получают revisions и состояние checkout через Git. Записывающие
команды не зависят от task progress; `status` читает checkbox только через публичный
JSON API OpenSpec и показывает его рядом с revision. Локальная запись начала работы
защищает историю и конкурентное обновление; опубликованный checkpoint переносится
между checkout. Запись того же task ID устаревшим исполнителем отклоняется, а разные
task ID обновляются параллельно.
