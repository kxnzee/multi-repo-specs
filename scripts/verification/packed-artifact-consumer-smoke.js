...
if (typeof npmCli !== "string" || !path.isAbsolute(npmCli)) {
  throw new Error("PACKED_CONSUMER_NPM_UNAVAILABLE: запустите через npm run test:pack:consumer");
}

const installTimeout = Number(process.env.PACKED_CONSUMER_INSTALL_TIMEOUT_MS ?? 600000);
if (!Number.isSafeInteger(installTimeout) || installTimeout <= 0) {
  throw new Error("PACKED_CONSUMER_INSTALL_TIMEOUT_INVALID: укажите положительное число миллисекунд");
}
...
try {
  ...
  try {
    runNpm([
      ...
      "--no-audit",
      "--no-fund",
    ], { cwd: consumer, env: npmEnvironment, stdio: "inherit", timeout: installTimeout });
  } catch (error) {
    if (error?.code === "ETIMEDOUT") {
      throw new Error(
        `PACKED_CONSUMER_REGISTRY_TIMEOUT: npm install не завершился за ${installTimeout} мс; `
        + "проверьте доступ к npm registry или увеличьте PACKED_CONSUMER_INSTALL_TIMEOUT_MS",
        { cause: error },
      );
    }
    throw error;
