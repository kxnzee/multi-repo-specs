...
Замените пути и адреса удалённых Git-репозиториев своими. Store должен быть отдельным чистым
Git-репозиторием с настроенным `origin`; не запускайте `init` в исходниках
...
git init -b main
git remote add origin ssh://git.example.org/product/specs.git
...
Существующие рабочие копии он не обновляет через `git pull`. Результат `needs_setup_pr`
означает, что добавленные файлы OpenSpec нужно принять через PR настройки репозитория (setup PR).
