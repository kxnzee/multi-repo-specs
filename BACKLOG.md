...
Текущая реализация и её границы описаны в
[технической документации](docs2/technical/README.md) и изменяются только через явное
решение владельца. Задачи ниже не являются планом текущей разработки и не блокируют
принятые изменения Core или Plugin SDK.
...
**Триггер:** подтверждено, что локальное состояние мешает передаче работы или
совместной проверке.
...
**Триггер:** команде требуется разделяемое доказательство хотя бы одного Gate.
...
**Триггер:** подтверждена ручная потеря или неоднозначность ссылок между этапами.
...
**Триггер:** пользователи регулярно ошибаются в следующем действии.
...
При подтверждённой потребности спроектировать безопасный batch-режим для десятков и
сотен repositories, агрегированную свежесть индексов и ограничение параллелизма. Сам индекс и реализация
...
**Триггер:** подтверждённое наблюдение первого запуска от 2026-08-17: пользователь
...
## Приоритизация по evidence

Для каждого кандидата собрать подтверждённые наблюдения, частоту, стоимость ручного
обхода и риск ошибки. Текущие номера не задают очередь реализации.
