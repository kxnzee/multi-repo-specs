...
После работы снова проверь, что `spec_root` чист и остаётся на том же `spec_baseline`. Любое изменение immutable Store блокирует завершение и требует повторного `openspec-orch load` после устранения причины.
