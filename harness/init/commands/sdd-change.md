...
Не выдавай предположение за факт, не закрепляй преждевременно реализацию и не переноси историю чата. Записывай только в проверенный `resolvedOutputPath`. Если `proposalStatus: present`, сначала прочитай существующий Proposal и не перезаписывай подтверждённый текст без явного изменения Change Owner.
