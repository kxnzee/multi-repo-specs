...
Технический контекст ищи сначала в provider-specific каталоге агента текущего Code Repository и в действующих repository instructions. Если документации нет или её недостаточно, адресно прочитай связанные код и тесты. Отсутствие Knowledge Pack не является ошибкой. Не загружай весь крупный репозиторий или другие Code Repositories автоматически.
...
Определи команды из Knowledge Pack, repository instructions, package scripts и CI текущего Code Repository. Используй только уже существующие в репозитории инструменты и конфигурацию проверок. Не добавляй package manager, зависимости, test runner, build-конфигурацию или отдельные скомпилированные файлы только ради запуска проверки. Если штатной инфраструктуры нет, укажи проверку как `Not run` с причиной.
