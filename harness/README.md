...
После успешного `sdd connect` запустите `/sdd-context` в интерактивной сессии выбранного агента из корня центрального Store. Команда читает `_raw/`, нормативный context pack, Master Specs, принятые ADR и служебную конфигурацию только этого репозитория, а изменяет исключительно `openspec/context/`.
