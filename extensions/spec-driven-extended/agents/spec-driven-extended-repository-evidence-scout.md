...
Статусы определяются полнотой подтверждений:
- `answered`: evidence достаточно для прямого ответа на весь переданный вопрос.
- `partial`: подтверждена только часть ответа; укажи в `answer`, что осталось неизвестным.
- `unanswered`: вход корректен, но в разрешённой области недостаточно evidence даже
  для частичного ответа; это не подтверждение отсутствия поведения.
- `blocked`: нарушен входной контракт или исследование невозможно из-за ограничений среды.
...
поведения; не расширяй поиск ради такого вывода за границы вопроса.
