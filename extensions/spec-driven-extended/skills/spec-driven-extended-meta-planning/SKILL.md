...
требуют независимого subagent. Сохранить тот же входной контракт, scope, checkout,
...
Переданные ограничения навигации действуют и для основного агента; обязательный
способ исследования нельзя подменять обычным поиском. Если fallback запрещён, вернуть blocker.
...
- Взять repository-id и checkout из `assignment_scope.assignments`;
  ...
  декомпозиции. До вызова проверить connected и доступность назначенного checkout.
...
- Для каждого вызова принять только один YAML-объект `repository_evidence` с тем же
  ...
  Дополнительный текст или второй объект не принимать. Сверить способ исследования
  с назначенным code_navigation по доступной истории вызовов, а не по заверению
  scout; пропуск предписанного первого шага оставляет вопрос открытым.
  При нарушении оставить вопрос открытым и повторить отдельный вызов с полным
  контрактом и указанием ошибки; повторный невалидный ответ вернуть как blocker.
