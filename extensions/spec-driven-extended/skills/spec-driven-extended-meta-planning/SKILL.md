...
- Взять repository-id, checkout и revision из `assignment_scope.assignments`;
  ...
  Адресно проверить наличие `.codegraph/` в checkout и передать `code_navigation`:
  `codegraph` при наличии индекса, `unindexed` при его отсутствии. Если индекс есть,
  а CodeGraph MCP не доставлен, вернуть blocker до вызова scout.
- Для каждого вызова принять только один YAML-объект `repository_evidence` с тем же
  ...
  scout; чтение кода до CodeGraph в индексированном checkout оставляет вопрос открытым.
