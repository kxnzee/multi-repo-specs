...
Proposal и Specs являются Store-only стадиями. Code Repository и
...
2. Checklist стадии составлять из `artifact_instructions.instruction` и применимых
   ...
   Для Design прочитай полностью актуальный `template`: он владеет правилами
   содержания и полноты контракта. Сначала проверь документ по ним; очевидное
   нарушение не требует нового repository scout. Положительный результат требует
   завершения всех применимых проверок, а не только успешной OpenSpec validation.
...
Для обратной сверки Intake и существующих артефактов используй
[spec-driven-extended-reconcile](../spec-driven-extended-reconcile/SKILL.md)
в режиме `review`. Переиспользуй свежий результат сверки, если входы не менялись.
Сам meta-skill остаётся read-only: при расхождении верни точные файлы и разделы
вызывающему workflow. До устранения противоречий не возвращай ready; нерешённое
бизнес-решение требует request_owner_decision. Отсутствующий helper означает
недоступную проверку, а не разрешение объявить документы согласованными.
...
Change или долговечный Store context. Design проверяй по его текущему template,
не создавая здесь второй список правил документа. Если Proposal, Specs или Tasks содержат
...
spec-driven-extended-repository-evidence-scout. Основной агент не заменяет scout:
не открывает Code Repository, не читает checkout или указанные в evidence файлы и
не запускает инструменты навигации по коду. Если механизм запуска scout недоступен,
обязательный вход отсутствует или scout вернул `blocked`, вернуть blocker.
...
- До первого вызова прочитать [полный профиль scout](../../subagents/spec-driven-extended-repository-evidence-scout.md)
  из этого установленного Extension (путь относительно каталога данного skill).
  Передавать YAML-объект `repository_evidence_request` по схеме профиля, включая
  `checkout_path`. Проверить обязательные поля до запуска; краткое описание
  subagent не содержит полного контракта.
- Взять repository-id и checkout из `assignment_scope.assignments`;
  ...
  декомпозиции. Repository-id и checkout принять из `assignment_scope` без обращения
  к checkout; его доступность проверяет только scout.
  В `code_navigation` передать самодостаточные правила навигации из активных
  инструкций проекта: первый шаг, параметры инструментов, ограничения и fallback.
  Если специальных правил нет, передать адресное чтение anchors и связанный поиск.
- Для каждого вызова принять только один YAML-объект `repository_evidence` с тем же
  question_id и полями status, answer и evidence. Если весь объект заключён
  в один Markdown-блок yaml, снять только это ограждение. Проверить, что внутри
  один YAML-объект, затем структуру evidence и допустимый status по профилю.
  Дополнительный текст или второй объект не принимать. Это только формальная
  валидация response contract: не перепроверять факты, sources или способ исследования
  через Code Repository, checkout, историю инструментов или повторную навигацию.
  Невалидный или недостаточный ответ оставить открытым и повторить отдельный вызов
  с полным контрактом и указанием ошибки; повторный невалидный ответ вернуть как blocker.
- `partial` использовать только для подтверждённой части вывода; неизвестную часть
  оставить открытой. `unanswered` и `blocked` не подтверждают отсутствие поведения
  и не закрывают проверку. Применять определения статусов из профиля scout.
...
Если проверка не завершена, инструкции недоступны или выполнение прервано,
не возвращай ready и не разрешай следующий этап. Укажи непроверенную часть.
OpenSpec status `done` означает наличие артефакта, а не завершённое review.
Результат остаётся в ответе агента, отдельный файл проверки не создаётся.
