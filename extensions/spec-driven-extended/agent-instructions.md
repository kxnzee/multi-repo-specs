...
работает с артефактами и принимает evidence от назначенного Repository scout.
Он валидирует только контракт ответа, не перепроверяет код. OpenSpec определяет
порядок стадий и требования к их результатам.
...
Эта формальная проверка не разрешает основному агенту открывать checkout, читать
указанные в evidence файлы, запускать инструменты навигации по коду или
перепроверять вывод scout. Невалидный или недостаточный ответ требует нового
вызова scout либо blocker.
...
Store context, выполняет Planning review и синтезирует принятые evidence;
