...
export function localState(value) {
  ...
  for (const session of value.sessions) {
    if (!shape(session, ["change_id", "repository_id", "task_id", "checkout_path", "store_revision",
      "base_revision", "observed", "last_saved", "active"]) ||
      !identifier(session.change_id) || !identifier(session.repository_id) ||
      !nonempty(session.task_id) ||
      !nonempty(session.checkout_path) || !path.isAbsolute(session.checkout_path) ||
      !revision(session.store_revision) || !revision(session.base_revision) ||
